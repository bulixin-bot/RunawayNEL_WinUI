...
using Runaway.Handlers.PC;
using Runaway.Handlers.Game.NetServer;
    ...
    public static async Task<BridgeResponse> ListSessions(BridgeRequest req)
    {
        try
        {
            ...
            var items = result.GetType().GetProperty("items")?.GetValue(result)
                as IEnumerable<Runaway.Entities.Web.NEL.EntityQueryGameSessions>;

            var sessions = (items ?? Enumerable.Empty<Runaway.Entities.Web.NEL.EntityQueryGameSessions>())
                .Select(s => new
                {
                    id = s.Id,
