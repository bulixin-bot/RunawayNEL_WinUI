...
using Runaway.Handlers.PC.Account;
using Serilog;
...
public static class AccountHandler
{
    public static async Task<BridgeResponse> GetAccounts(BridgeRequest req)
    {
        try
        {
            await Runaway.Backend.WaitForInitAsync();
    ...
    public static async Task<BridgeResponse> RandomLogin(BridgeRequest req)
    {
        try
        {
            var (success, message) = await Runaway.Backend.RandomLogin4399WithCredentialsAsync();
            return success
        ...
        try
        {
            ...
            switch (method)
            {
                case "4399":
                {
                    ...
                    result = await Runaway.Backend.Login4399Async(account, password);
                    break;
                }
                case "netease":
                {
                    ...
                    result = await Runaway.Backend.LoginNeteaseAsync(email, password);
                    break;
                }
                case "cookie":
                {
                    ...
                    result = await Runaway.Backend.LoginCookieAsync(cookie);
                    break;
                }
                default:
